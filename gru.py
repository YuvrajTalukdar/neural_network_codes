...
def create_network(data_arr,reshaped_training_data,no_of_classes):
    model = Sequential()
    model.add(GRU(45,input_shape=(reshaped_training_data.shape[1],1),activation='relu',return_sequences=True))
    model.add(Dropout(0.2))

    model.add(GRU(45,activation='relu',return_sequences=True))
    model.add(Dropout(0.2))

    model.add(GRU(int(len(data_arr[0])/2),activation='relu',return_sequences=False))
    model.add(Dropout(0.2))
